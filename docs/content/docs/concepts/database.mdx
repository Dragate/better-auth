...
The check runs outside production and keeps its verdict for the life of the process. The built-in Kysely adapter introspects the database once. Kysely plugins that rename identifiers, such as `CamelCasePlugin`, are taken into account, so a snake\_case database compares correctly. The Drizzle adapter reads the schema object it was given and the Prisma adapter reads the generated client's data model, so neither sends a query. The data model the default `prisma-client` generator emits carries names but not nullability, so with Prisma a required column Better Auth never fills is only reported by `npx auth generate`. `npx auth migrate` reports the same problems before it changes anything.
